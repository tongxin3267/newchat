...
<form method="post" return="<?= $return_url ?>" class="auto-form card">
    ...
    <div class="card-body">
        ...
        <div class="form-group row">
            <label class="col-sm-3 col-form-label">审核权限</label>
            <div class="col-sm-6">
                <input id="refer" class="form-control" value="<?= $model->refer ?>"
                       name="refer" hidden>
                <label class="custom-control custom-checkbox ml-3">
                    <?php if ($model->refer == 1) : ?>
                    <input type="checkbox" class="custom-control-input refer" checked>
                    <?php else : ?>
                    <input type="checkbox" class="custom-control-input refer">
                    <?php endif; ?>
                    <span class="custom-control-indicator"></span>
                    <span class="custom-control-description">提成审核权限</span>
                </label>
            </div>
        </div>
        <div class="form-group row">
...
<script>
    ...
    $(document).on('change', '.no-expire-time', function () {
 	...
 	$(document).on('change', '.refer', function () {
        if ($(this).prop('checked')) {
            $("#refer").val(1);
