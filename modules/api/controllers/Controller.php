...
class Controller extends \app\controllers\Controller
{
   ...
   public $wechats;
    ...
    public function init()
    {
        ...
        $this->wechats = WechatApp::find()->where(['id'=>$this->store->wechat_app_id])->one();
      	$this->appid = $this->wechats->app_id;
     	 $this->appsecret = $this->wechats->app_secret;
        if (!$this->wechats) {
        ...
        $cert_pem_file = null;
        if ($this->wechats->cert_pem) {
            $cert_pem_file = \Yii::$app->runtimePath . '/pem/' . md5($this->wechats->cert_pem);
            if (!file_exists($cert_pem_file)) {
                file_put_contents($cert_pem_file, $this->wechats->cert_pem);
            }
        }

        $key_pem_file = null;
        if ($this->wechats->key_pem) {
            $key_pem_file = \Yii::$app->runtimePath . '/pem/' . md5($this->wechats->key_pem);
            if (!file_exists($key_pem_file)) {
                file_put_contents($key_pem_file, $this->wechats->key_pem);
            }
        }

        $this->wechat = new Wechat([
            'appId' => $this->wechats->app_id,
            'appSecret' => $this->wechats->app_secret,
            'mchId' => $this->wechats->mch_id,
            'apiKey' => $this->wechats->key,
