        ...
        if ($this->validate()) {
            ...
            if ($goods->save()) {
				...
				if($this->mall_id){
					...
					if($goods_relation){
						$ms_goods_relation=PrinceGoodsRelation::findOne(['goods_id' => $goods->id, 'type' => 1, 'store_id' => $this->store_id]);
						if(!$ms_goods_relation){
							$ms_goods_relation=new PrinceGoodsRelation();
						}
						$ms_goods_relation->store_id = $this->store_id;
						$ms_goods_relation->cloud_goods_id = $goods_relation->cloud_goods_id;
						$ms_goods_relation->goods_id = $goods->id;
						$ms_goods_relation->type = 1;
						$ms_goods_relation->save(false);
					}
				}
				//P_ADD end
                return [
