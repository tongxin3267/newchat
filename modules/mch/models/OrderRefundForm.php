    ...
    private function submit1($order_refund)
    {
        ...
        if ($this->action == 2) {//拒绝
            ...
            if ($order_refund->save()) {
                ...
                if ($this->orderType === self::MS) {
                    $msg_sender = new MsWechatTplMsgSender($this->store_id, $order_refund->order_id, $this->getWechat());
                    $msg_sender->refundMsg('0.00', $order_refund->goods->name, '卖家拒绝了您的退货请求');

                } else if ($this->orderType === self::PT) {
                    $msg_sender = new PtNoticeSender($this->getWechat(), $this->store_id);
                    $msg_sender->refundMsg($order->id, '0.00', $order_refund->goods->name, '卖家拒绝了您的退货请求');
    ...
    private function submit2($order_refund)
    {
        ...
        if ($this->action == 2) {//拒绝
            ...
            if ($order_refund->save()) {
                if ($this->orderType === self::MS) {
                    $msg_sender = new MsWechatTplMsgSender($this->store_id, $order_refund->order_id, $this->getWechat());
                    $msg_sender->refundMsg('0.00', $order_refund->goods->name, '卖家已拒绝您的换货请求');

                } else if ($this->orderType === self::PT) {
                    $msg_sender = new PtNoticeSender($this->getWechat(), $this->store_id);
                    $msg_sender->refundMsg($order_refund->order_id, '0.00', $order_refund->goods->name, '卖家已拒绝您的换货请求');
