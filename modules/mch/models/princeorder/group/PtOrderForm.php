...
namespace app\modules\mch\models\princeorder\group;//P_MOD
...
class PtOrderForm extends MchModel
{
    ...
    public function getList()
    {
        ...
        foreach ($list as $k => $v) {
            if ($v['shop_id']) {
                $list[$k]['shop'] = Shop::find()->select(['name', 'mobile', 'address', 'longitude', 'latitude'])->where([ 'id' => $v['shop_id']])->asArray()->one();
            }
            if ($v['order_no'] == 'robot') {
    ...
    public function getGroupList()
    {
        ...
        $OrderCountQuery = PtOrder::find()->andWhere(['is_delete' => 0, 'is_group' => 1])
        ...
        $query = PtOrderDetail::find()
            ...
            ->andWhere(['o.is_delete' => 0,  'o.is_group' => 1, 'o.parent_id' => 0])
            ->andWhere(['or', ['o.is_pay' => 1], ['o.pay_type' => 2]])
        ...
        foreach ($list as $key => $item) {
            if ($item['class_group']) {
                $detail = PtGoodsDetail::find()->where(['id' => $item['class_group']])->select(['group_num', 'colonel'])->one();
    ...
    public function getGroupInfo($pid = 0)
    {
        ...
        foreach ($list as $key => $value) {
            if ($value['class_group']) {
                $detail = PtGoodsDetail::findone(['id' => $value['class_group']]);
                $list[$key]['group_num'] = $detail->group_num;
                $list[$key]['colonel'] = $detail->colonel;
            }
