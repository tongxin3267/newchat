...
namespace app\modules\mch\models\princeorder\integralmall;
...
class IntegralOrderForm extends MchModel
{
    ...
    {
        $query = IntegralOrder::find()->alias('o')
            ->where(['o.is_delete' => 0])
        ...
        switch ($this->status) {
            ...
            case '2'://待收货
                $query->andWhere([
                    'o.is_send' => 1,
                    'o.is_confirm' => 0,
                ])->andWhere(['or',['o.is_pay'=>1],['o.pay_type'=>1]]);
                break;
            case '3'://已完成
                $query->andWhere([
                    'o.is_send' => 1,
    ...
    public function save()
    {
        ...
        if ($order->save()) {
            try {
                $wechat_tpl_meg_sender = new WechatTplMsgSender($order->store_id, $order->id, $this->getWechat());
        ...
        if ($order->save()) {
            ...
            if ($goods->save()) {
                ...
                if ($order_detail->save()) {
                    $msg_sender = new WechatTplMsgSender($order->store_id, $order->id, $this->getWechat());
                    if ($order->is_pay) {
