...
namespace app\modules\mch\models\princeorder;
...
class OrderListForm extends MchModel
{
    ...
    public function search()
    {
        ...
        $query = Order::find()->alias('o')->where(['o.mch_id'   => 0])
        ...
        $list = $query->limit($pagination->limit)->offset($pagination->offset)->orderBy('o.addtime DESC')
            ->select(['o.*','s.name AS store_name', 'u.nickname', 'u.platform', 'clerk_name' => $clerkQuery, 'refund' => $refundQuery])->asArray()->all();
