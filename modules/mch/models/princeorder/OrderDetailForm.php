...
namespace app\modules\mch\models\princeorder;//P_MOD
...
use app\models\User;
use app\modules\mch\models\MchModel;//P_ADD
...
class OrderDetailForm extends MchModel
{
    ...
    {
        $order = Order::find()->where(['id' => $this->order_id])->asArray()->one();
        if (!$order) {
            return [
                'code'=>1,
                'msg'=>'fail'
        ...
        $order_form = OrderForm::find()->where(['order_id' => $order['id'], 'is_delete' => 0])->asArray()->all();
        $order_refund = OrderRefund::findOne(['order_id' => $order['id'], 'is_delete' => 0]);
        if ($order_refund) {
