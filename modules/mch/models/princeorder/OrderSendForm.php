...
class OrderSendForm extends MchModel
{
    ...
    public function batch($arrCSV)
    {
        ...
        foreach ($arrCSV as $v) {
            ...
            if (!$order->save()) {
                ...
                $success[] = $v[1];
                try {
                          $wechat_tpl_meg_sender = new WechatTplMsgSender($order->store_id, $order->id, $this->getWechat());//P_MOD
    ...
    private function sendMessage($order)
    {
        ...
        $store = Store::findOne($order->store_id);//P_MOD
