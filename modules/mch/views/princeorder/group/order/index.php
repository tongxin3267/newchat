                ...
                <form method="get">
                        ...
                        <div class="mr-4">
                            <div class="form-group">
                                ...
                                <a class="btn btn-secondary del"
                                   href="javascript:"
                                   data-url="<?= $urlManager->createUrl(['mch/princeorder/group/order/delete-all']) ?>"
                    ...
                    <div flex="dir:left">
                        ...
                        <div class="dropdown float-left">
                            ...
                            <div class="dropdown-menu" style="min-width:8rem">
                                <a class="dropdown-item"
                                   href="<?= $urlManager->createUrl(['mch/princeorder/group/order/index']) ?>">所有订单</a>
                                <a class="dropdown-item"
                                   href="<?= $urlManager->createUrl(['mch/princeorder/group/order/index', 'offline' => 1]) ?>">非自提</a>
                                <a class="dropdown-item"
                                   href="<?= $urlManager->createUrl(['mch/princeorder/group/order/index', 'offline' => 2]) ?>">自提</a>
                            </div>
                        </div>
                    </div>
                </form>
            ...
            <ul class="nav nav-tabs status">
                <li class="nav-item">
                    <a class="status-item nav-link <?= $status == -1 ? 'active' : null ?>"
                       href="<?= $urlManager->createUrl(array_merge(['mch/princeorder/group/order/index'])) ?>">全部</a>
                </li>
                <li class="nav-item">
                    <a class="status-item nav-link <?= $status == 0 ? 'active' : null ?>"
                       href="<?= $urlManager->createUrl(array_merge(['mch/princeorder/group/order/index'], $condition, ['status' => 0])) ?>">未付款<?= $store_data['status_count']['status_0'] ? '(' . $store_data['status_count']['status_0'] . ')' : null ?></a>
                </li>
                <li class="nav-item">
                    <a class="status-item nav-link <?= $status == 1 ? 'active' : null ?>"
                       href="<?= $urlManager->createUrl(array_merge(['mch/princeorder/group/order/index'], $condition, ['status' => 1])) ?>">待发货<?= $store_data['status_count']['status_1'] ? '(' . $store_data['status_count']['status_1'] . ')' : null ?></a>
                </li>
                <li class="nav-item">
                    <a class="status-item nav-link <?= $status == 2 ? 'active' : null ?>"
                       href="<?= $urlManager->createUrl(array_merge(['mch/princeorder/group/order/index'], $condition, ['status' => 2])) ?>">已发货<?= $store_data['status_count']['status_2'] ? '(' . $store_data['status_count']['status_2'] . ')' : null ?></a>
                </li>
                <li class="nav-item">
                    <a class="status-item nav-link <?= $status == 6 ? 'active' : null ?>"
                       href="<?= $urlManager->createUrl(array_merge(['mch/princeorder/group/order/index'], $condition, ['status' => 6])) ?>">拼团中<?= $store_data['status_count']['status_6'] ? '(' . $store_data['status_count']['status_6'] . ')' : null ?></a>
                </li>
                <li class="nav-item">
                    <a class="status-item nav-link <?= $status == 3 ? 'active' : null ?>"
                       href="<?= $urlManager->createUrl(array_merge(['mch/princeorder/group/order/index'], $condition, ['status' => 3])) ?>">已完成<?= $store_data['status_count']['status_3'] ? '(' . $store_data['status_count']['status_3'] . ')' : null ?></a>
                </li>
                <li class="nav-item">
                    <a class="status-item nav-link <?= $status == 5 ? 'active' : null ?>"
                       href="<?= $urlManager->createUrl(array_merge(['mch/princeorder/group/order/index'], $condition, ['status' => 5])) ?>">已取消<?= $store_data['status_count']['status_5'] ? '(' . $store_data['status_count']['status_5'] . ')' : null ?></a>
                </li>

                <li class="nav-item">
                    <a class="status-item nav-link <?= $status == 4 ? 'active' : null ?>"
                       href="<?= $urlManager->createUrl(array_merge(['mch/princeorder/group/order/index'], $condition, ['status' => 4])) ?>">拼团失败</a>
                </li>
                <li class="nav-item">
                                ...
                                <?php if ($order_item['is_cancel'] == 0): ?>
                                    ...
                                    <span class="ml-1">
                                        <a class="btn btn-sm btn-info agree apply-status-btn"
                                           href="<?= $urlManager->createUrl(['mch/princeorder/group/order/apply-delete-status', 'id' => $order_item['id'], 'status' => 1]) ?>">同意请求</a>
                                    </span>
                                    <span class="ml-1">
                                        <a class="btn btn-sm btn-danger refuse apply-status-btn"
                                           href="<?= $urlManager->createUrl(['mch/princeorder/group/order/apply-delete-status', 'id' => $order_item['id'], 'status' => 0]) ?>">拒绝请求</a>
                                    </span>
                                <?php endif; ?>
                            <?php endif; ?>
                        </td>
                        ...
                        <td class="order-tab-5">
                            ...
                            <?php if (($order_item['is_pay'] == 1 || $order_item['pay_type'] == 2) && $order_item['offline'] == 2 && $order_item['is_send'] != 1) : ?>
                                    <a class="btn btn-sm btn-danger mt-2  del" href="javascript:"
                                       data-content="请到产生该订单的订单列表菜单进行核销">核销</a>
                                </div>
                            <?php endif; ?>
                            <div>
                            ...
                            <?php if ($order_item['is_send'] == 1 && $order_item['pay_type'] == 2 && $order_item['offline'] != 1): ?>
                                <div>
                                   ...
                                   data-url="<?= $urlManager->createUrl(['mch/princeorder/group/order/confirm', 'order_id' => $order_item['id']]) ?>"
                            ...
                            <?php if ($order_item['is_recycle'] == 1) : ?>
                                <div>
                                   ...
                                   data-url="<?= $urlManager->createUrl(['mch/princeorder/group/order/recycle', 'order_id' => $order_item['id'], 'is_recycle' => 0]) ?>"
                                   data-content="是否移出回收站">移出回收站</a>
                                </div>
                                <div>
                                <a class="btn btn-sm btn-danger del mt-2" href="javascript:"
                                   data-url="<?= $urlManager->createUrl(['mch/princeorder/group/order/delete', 'order_id' => $order_item['id']]) ?>"
                                       data-content="是否删除">删除订单</a>
                                </div>
                            <?php else : ?>
                                <div>
                                    <a class="btn btn-sm btn-danger mt-2 del" href="javascript:"
                                   data-url="<?= $urlManager->createUrl(['mch/princeorder/group/order/recycle', 'order_id' => $order_item['id'], 'is_recycle' => 1]) ?>"
...
<script>
    ...
    $(document).on("click", ".send-confirm-btn", function () {
        ...
        console.log(error);
        $.ajax({
            url: "<?=$urlManager->createUrl(['mch/princeorder/group/order/send'])?>",
            type: "post",
...
<script>
    ...
    $(document).on('click', '.print', function () {
        ...
        $.ajax({
            url: "<?=$urlManager->createUrl(['mch/princeorder/group/order/print'])?>",
            type: 'get',
            dataType: 'json',
            data: {
                id: id,
