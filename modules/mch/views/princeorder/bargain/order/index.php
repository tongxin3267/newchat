                        ...
                        <div class="mr-4">
                            <div class="form-group">
                                ...
                                <a class="btn btn-secondary del"
                                   href="javascript:"
                                   data-url="<?= $urlManager->createUrl(['mch/princeorder/bargain/order/delete-all']) ?>"
                ...
                <table class="table table-bordered bg-white">
                    <tr>
                        <td colspan="5">
                            <span class="mr-3">
                            <span
                                        class='titleColor'>商城：</span><?= $order_item['store_name'] ?></span>
                            <span
                                ...
                                <?php if ($order_item['is_delete'] == 0) : ?>
                                    <div class="mb-2">
                                        <a class="btn btn-sm btn-primary apply-status-btn"
                                           href="<?= $urlManager->createUrl(['mch/princeorder/order/apply-delete-status', 'id' => $order_item['id'], 'status' => 1]) ?>">同意请求</a>
                                    </div>
                                    <div class="mb-2">
                                        <a class="btn btn-sm btn-danger apply-status-btn"
                                           href="<?= $urlManager->createUrl(['mch/princeorder/order/apply-delete-status', 'id' => $order_item['id'], 'status' => 0]) ?>">拒绝请求</a>
                                    </div>
                                <?php endif; ?>
                            <?php endif; ?>
                           ...
                           <?php if (($order_item['is_pay'] == 1 || $order_item['pay_type'] == 2) && $order_item['is_confirm'] != 1) : ?>
                            ...
                            <?php if (($order_item['is_pay'] == 1 || $order_item['pay_type'] == 2) && $order_item['is_offline'] == 1 && $order_item['is_send'] != 1) : ?>
                                    <a class="btn btn-sm btn-danger mt-2  del" href="javascript:"
                                       data-content="请到产生该订单的“应用专区->九宫格抽奖->赠品订单”菜单进行核销">核销</a>
                                </div>
                            <?php endif; ?>
                            <div>
                                <a class="btn btn-sm btn-primary mt-2"
                               href="<?= $urlManager->createUrl(['mch/princeorder/order/detail', 'order_id' => $order_item['id']]) ?>">详情</a>
                                <a class="btn btn-sm btn-primary mt-2 about"
                                   href="javascript:" data-toggle="modal"
                                   data-target="#about" data-id="<?= $order_item['id'] ?>" data-remarks="<?= $order_item['seller_comments'] ?>" data-url="<?=$urlManager->createUrl(['mch/princeorder/order/seller-comments', 'order_id' => $order_item['id']])?>">添加备注</a>
                            </div>
                            <?php if ($order_item['pay_type'] == 2 && $order_item['is_send'] == 1 && $order_item['is_confirm'] != 1) : ?>
                                ...
                                <a href="javascript:" class="btn btn-sm btn-primary del"
                                   data-url="<?= $urlManager->createUrl(['mch/princeorder/order/confirm', 'order_id' => $order_item['id']]) ?>"
                            ...
                            <?php if ($_GET['is_recycle'] == 1) : ?>
                                <div>
                                   ...
                                   data-url="<?= $urlManager->createUrl(['mch/princeorder/order/edit', 'order_id' => $order_item['id'], 'is_recycle' => 0]) ?>"
                                   data-content="是否移出回收站">移出回收站</a>
                                </div>
                                <div>
                                <a class="btn btn-sm btn-danger del mt-2" href="javascript:"
                                   data-url="<?= $urlManager->createUrl(['mch/princeorder/order/delete', 'order_id' => $order_item['id']]) ?>"
                                       data-content="是否删除">删除订单</a>
                                </div>
                            <?php else : ?>
                                <div>
                                    <a class="btn btn-sm btn-danger mt-2 del" href="javascript:"
                                   data-url="<?= $urlManager->createUrl(['mch/princeorder/order/edit', 'order_id' => $order_item['id'], 'is_recycle' => 1]) ?>"
...
<script>
    ...
    $(document).on("click", ".send-confirm-btn", function () {
        ...
        console.log(error);
        $.ajax({
            url: "<?=$urlManager->createUrl(['mch/princeorder/order/send'])?>",
            type: "post",
...
<script>
    ...
    $(document).on('click', '.print', function () {
        ...
        $.ajax({
            url: "<?=$urlManager->createUrl(['mch/princeorder/order/print'])?>",
            type: 'get',
            dataType: 'json',
            data: {
                id: id,
...
<script>
    ...
    $(document).on('click', '.add-price', function () {
        ...
        error.hide();
        $.ajax({
            url: "<?=$urlManager->createUrl(['mch/princeorder/order/add-price'])?>",
            type: 'get',
            dataType: 'json',
            data: {
                order_id: order_id,
