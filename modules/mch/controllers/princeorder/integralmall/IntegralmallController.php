...
namespace app\modules\mch\controllers\princeorder\integralmall;
...
use app\modules\mch\models\princeorder\integralmall\IntegralOrderForm;
    ...
    public function actionApplyDeleteStatus($id, $status)
    {
        ...
        if ($status == 1) { //同意
			if($order->store_id !==$this->store->id){
                return [
                    'code' => 1,
                    'msg' => '对不起，请到产生该订单的商城“应用专区->积分商城->订单列表”菜单进行操作。',
                ];
			}
            $form = new IntegralOrderForm();
            ...
            $order->save();
            $msg_sender = new WechatTplMsgSender($order->store_id, $order->id, $this->wechat);
