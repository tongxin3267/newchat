...
namespace app\modules\mch\controllers\princeorder;
...
use app\modules\mch\models\princeorder\ExportList;//P_MOD
use app\modules\mch\models\order\OrderClerkForm;
use app\modules\mch\models\princeorder\OrderDeleteForm;//P_MOD
use app\modules\mch\models\princeorder\OrderDetailForm;//P_MOD
use app\modules\mch\models\princeorder\OrderListForm;//P_MOD
...
use app\modules\mch\models\princeorder\OrderSendForm;//P_MOD
...
class OrderController extends Controller
{
    public function actionIndex($is_offline = null)
    {
        ...
        if ($clerk_id) {
            $clerk = User::findOne(['id' => $clerk_id]);
        }
        if ($shop_id) {
    ...
    public function actionEdit()
    {
        ...
        if ($is_recycle == 0 || $is_recycle == 1) {
            $form = Order::find()->where([ 'mch_id' => 0])
    ...
    public function actionSellerComments()
    {
        ...
        $form = Order::find()->where(['id' => $order_id, 'mch_id' => 0])->one();
    ...
    public function actionSend()
    {
        ...
        //$form->store_id = $this->store->id;//P_MOD
        return $form->save();
    }
    ...
    public function actionApplyDeleteStatus($id, $status)
    {
        ...
        if ($status == 1) { //同意
			if($order->store_id !==$this->store->id){
                return [
                    'code' => 1,
                    'msg' => '对不起，改操作无法在订单汇总菜单操作，请到产生该订单的商城的对应菜单进行操作。',
                ];
			}
            ...
            $order->save();
            $msg_sender = new WechatTplMsgSender($order->store_id, $order->id, $this->wechat);//P_MOD
