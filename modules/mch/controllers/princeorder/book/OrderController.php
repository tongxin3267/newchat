...
namespace app\modules\mch\controllers\princeorder\book;

use app\models\YyOrder;
use app\models\YyWechatTplMsgSender;
use app\modules\mch\models\princeorder\book\OrderForm;
use app\modules\mch\models\ExportList;
use app\models\User;
use app\models\UserAccountLog;
use app\modules\mch\models\order\OrderClerkForm;
use app\modules\mch\models\princeorder\OrderDeleteForm;
...
class OrderController extends Controller
{
    ...
    public function actionRefund()
    {
        ...
        if($type == 2){
			...
			if($order->store_id !==$this->store->id){
                return [
                    'code' => 1,
                    'msg' => '对不起，请到产生该订单的商城“应用专区->预约->订单管理”菜单进行操作。',
                ];
			}
        /** @var Wechat $wechat */
    ...
    {
        $order = YyOrder::find()->where(['is_delete' => 0, 'id' => $order_id])->asArray()->one();
        if (!$order) {
    ...
    public function actionSellerComments()
    {
        ...
        $form = YyOrder::find()->where([ 'id' => $order_id])->one();
