...
namespace app\modules\mch\controllers\princeorder\group;//P_MOD
...
use app\modules\mch\models\princeorder\group\OrderSendForm;//P_MOD
use app\modules\mch\models\princeorder\group\PtOrderForm;//P_MOD
...
class OrderController extends Controller
{
    ...
    public function actionApplyDeleteStatus($id, $status)
    {
        ...
        if ($status == 1) { //同意
			if($order->store_id !==$this->store->id){
                return [
                    'code' => 1,
                    'msg' => '对不起，请到产生该订单的商城“应用专区->拼团管理->订单管理”菜单进行操作。',
                ];
			}
    ...
    public function actionDetail($order_id = null)
    {
        ...
        $order = PtOrder::find()->where(['is_delete' => 0,  'id' => $order_id])->asArray()->one();
        if (!$order) {
        ...
        $order_refund = PtOrderRefund::findOne(['order_id' => $order['id'], 'is_delete' => 0]);
        if ($order_refund) {
    ...
    public function actionSellerComments()
    {
        ...
        $form = PtOrder::find()->where([ 'id' => $order_id])->one();
